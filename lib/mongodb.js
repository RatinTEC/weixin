class MongoDB{
	...
	find(tablename,wherestr,field,callback,args=null){
		this.MongoClient.connect(this.DB_CONN_STR,function(err, db){
                        var collection = db.collection(tablename);
                        //插入数据
	...
	findandupdate_group(tablename,wherestr,updatestr,callback,args=null){
		this.MongoClient.connect(this.DB_CONN_STR,function(err, db){
                        var collection = db.collection(tablename);
                        //插入数据
			collection.aggregate(wherestr).toArray(function(err,result){
				if(args!=null){
					callback(result,args);
				}else{
					callback(result);
